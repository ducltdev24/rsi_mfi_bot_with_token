...
def get_trade_signal(rsi, mfi):
    if rsi <= 30 and mfi <= 20: return "🟢 LONG (quá bán)"
    if rsi >= 70 and mfi >= 80: return "🔴 SHORT (quá mua)"
    if rsi <= 30: return "🟢 RSI quá bán"
    if rsi >= 70: return "🔴 RSI quá mua"
    if mfi <= 20: return "🟢 MFI quá bán"
    if mfi >= 80: return "🔴 MFI quá mua"
...
RSI: {rsi:.2f} | MFI: {mfi:.2f}\n{signal}"
                results[symbol].append({
                    'msg': msg, 'price': price, 'interval': interval, 'signal': signal
                })
    ...
    for symbol, signals in results.items():
        ...
        if len(meaningful) >= 2:
            special_alert += f"⚡ {symbol}: NHIỀU KHUNG GIỜ CÙNG QUÁ MUA/QUÁ BÁN! ({', '.join([s['interval'] for s in meaningful])})\n"
        if meaningful:
            any_signal = True
            price = signals[0]['price']
            combined_msg = f"💱 {symbol} - Giá: ${price:.2f}\n\n" + "\n\n".join([s['msg'] for s in signals])
